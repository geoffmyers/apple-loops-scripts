...
# Verification
#
# A file can carry Apple Loop metadata, decode without error, and still not do
# what the metadata claims: not follow tempo, not appear under any Loop Browser
# filter, or stretch to the wrong length. These are the ways that happens.
# ---------------------------------------------------------------------------
...
# Outside this range the beat count is wrong, not the music. Bounds come from
# Apple's own library: across 34,928 shipped loops the derived tempo runs from
# 21.2 BPM to 360.0 BPM. An earlier 40-300 guess called 42 Apple loops broken.
PLAUSIBLE_TEMPO_RANGE = (20.0, 400.0)


def verify_apple_loop(info: 'AppleLoopInfo') -> List[Tuple[str, str]]:
    """Check a decoded file against what Logic actually needs.

    Returns a list of (severity, message), severity being "error" or
    "warning". An empty list means the file should behave natively.
    """
    findings: List[Tuple[str, str]] = []
    meta = info.metadata
    markers = info.beat_markers

    has_metadata = bool(meta.category or meta.genre or meta.beat_count
                        or meta.key_signature or meta.descriptors)
    if not has_metadata:
        # A warning, not an error: Apple ships 148 untagged loops of its own.
        findings.append(("warning",
                         "no Apple Loop metadata chunk: this is a plain audio "
                         "file, and the Loop Browser will not index it"))
        return findings

    # --- tempo following -------------------------------------------------
    has_markers = markers.marker_count > 0
    if meta.beat_count > 0 and not has_markers:
        findings.append(("error",
                         f"beat count is {meta.beat_count} but there are no "
                         f"beat markers: it will not follow project tempo"))
    if has_markers and meta.beat_count <= 0:
        # Unusual rather than broken: 563 of Apple's own loops do this.
        findings.append(("warning",
                         "beat markers are present but there is no beat count, "
                         "so Logic cannot derive a tempo"))

    duration = info.audio.duration or info.midi.duration
    if meta.beat_count > 0 and duration:
        derived = (meta.beat_count * 60.0) / duration
        low, high = PLAUSIBLE_TEMPO_RANGE
        if not (low <= derived <= high):
            findings.append(("error",
                             f"beat count {meta.beat_count} over {duration:.2f}s "
                             f"implies an implausible {derived:.0f} BPM"))

    # --- Loop Browser visibility ----------------------------------------
    if meta.category and meta.category not in APPLE_CATEGORIES:
        findings.append(("error",
                         f"category {meta.category!r} is not one Apple "
                         f"recognises, so no browser filter will show it"))
    if meta.subcategory and meta.subcategory not in APPLE_SUBCATEGORIES:
        findings.append(("warning",
                         f"subcategory {meta.subcategory!r} is not a known "
                         f"Apple value"))
    if meta.genre and meta.genre not in APPLE_GENRES:
        findings.append(("error",
                         f"genre {meta.genre!r} is not one Apple recognises"))
    # NOT checked: a genre in the info chunk. Only 21.6% of Apple's own loops
    # carry one, so its absence is normal and warning about it fired against
    # 78% of the shipped library.

    # --- key rules -------------------------------------------------------
    if meta.category in PERCUSSIVE_APPLE_CATEGORIES and meta.key_signature:
        findings.append(("warning",
                         f"{meta.category} loops should carry no key "
                         f"signature, but this one says {meta.key_signature!r}"))
    if meta.key_type and meta.key_type != 'neither' and not meta.key_signature:
        findings.append(("warning",
                         f"key type {meta.key_type!r} with no key signature"))

    # NOT checked: where the last beat marker sits relative to the audio.
    # Apple places markers on transients, not on a grid spanning the file --
    # across its library the last marker lands anywhere from 0.14x to 8.8x the
    # audio length. Requiring it to reach the end flagged 4,093 Apple loops.
...
Supported formats: CAF (modern Apple Loops), AIFF (legacy Apple Loops)
    ...
    parser.add_argument('--verify', action='store_true',
                        help='Check each file against what Logic actually '
                             'needs (tempo following, Loop Browser visibility, '
                             'marker alignment) and exit non-zero on errors')
    ...
    for file_path in files:
        try:
            ...
            if args.verify:
                ...
                if findings:
                    verify_findings[str(file_path)] = findings
                    verify_error_count += sum(
                        1 for severity, _ in findings if severity == 'error')
    ...
    # Verification report
    if args.verify and not args.json and not args.quiet:
        print()
        print("=" * 80)
        print("VERIFICATION")
        print("=" * 80)
        if not verify_findings:
            print(f"All {len(results)} file(s) look like native Apple Loops.")
        else:
            for path, findings in verify_findings.items():
                print(f"\n{Path(path).name}")
                for severity, message in findings:
                    marker = "ERROR  " if severity == 'error' else "warning"
                    print(f"  [{marker}] {message}")
            clean = len(results) - len(verify_findings)
            print(f"\n{clean}/{len(results)} clean, "
                  f"{verify_error_count} error(s) across "
                  f"{len(verify_findings)} file(s).")
    ...
    if args.json:
        ...
        if args.verify:
            output['verification'] = {
                path: [{'severity': s, 'message': m} for s, m in findings]
                for path, findings in verify_findings.items()
            }
            output['summary']['verification_errors'] = verify_error_count
    ...
    sys.exit(1 if errors or verify_error_count else 0)
