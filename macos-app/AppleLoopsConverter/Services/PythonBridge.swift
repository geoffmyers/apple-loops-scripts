...
class PythonBridge: ObservableObject {
    ...
    private var scriptPath: URL? {
        ...
        // 4. When running from Xcode DerivedData the app is nowhere near its
        //    source, so fall back to the toolkit this file was compiled from.
        //    #filePath resolves at compile time on the machine doing the build,
        //    which keeps a developer's own directory layout out of the source.
        //    This file lives at <toolkit>/macos-app/AppleLoopsConverter/Services/,
        //    so the toolkit root is four levels up.
        if Bundle.main.bundleURL.path.contains("DerivedData") {
            let sourceRelativeScript = URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()   // Services
                .deletingLastPathComponent()   // AppleLoopsConverter
                .deletingLastPathComponent()   // macos-app
                .deletingLastPathComponent()   // toolkit root
                .appendingPathComponent("convert_to_apple_loops.py")

            if FileManager.default.fileExists(atPath: sourceRelativeScript.path) {
                return sourceRelativeScript
            }
        }
